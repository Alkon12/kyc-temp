...
export const KYC_IDENTIFICATION_CARD_VERIFICATION: TaskTypeConfig = {
  accept: async (args, services) => {
    ...
    await services.applicationChecklistService.complete({
      ...
      checklistId: ChecklistId.IDENTIFICATION_CARD_VERIFICATION,
    });
  ...
  decline: async (args, services) => {
    ...
    const checklistEntry = await services.prisma.applicationChecklist.findFirst({
      where: {
        ...
        checklistId: "IDENTIFICATION_CARD",
      },
      orderBy: { id: 'asc' },
    });
    ...
    if (checklistEntry?.completedAt !== null) {
      const verification = await services.prisma.applicationChecklist.findFirst({
        where: {
          applicationId: applicationId._value,
          checklistId: "IDENTIFICATION_CARD_VERIFICATION",
        },
        orderBy: { id: 'asc' },
      });
    ...
    await services.notificationService.sendToUser(application.getUserId(), {
      ...
      title: new StringValue('Necesitamos que vuelvas a ingresar tu documento de identidad'),
      content: new StringValue(`Motivo: ${args.metadata?.message?.toDTO() ?? 'sin especificar'}`),
    });

    await sendFirebaseNotification(application.getUserId()?._value, 'Necesitamos que vuelvas a ingresar tu documento de identidad', `Motivo: ${args.metadata?.message?.toDTO() ?? 'sin especificar'}`, services);
    return true;
  },
};
