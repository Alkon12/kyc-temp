...
export const KYC_TAX_IDENTIFICATION_VERIFICATION: TaskTypeConfig = {
  ...
  decline: async (args, services) => {
    ...
    const checklistEntry = await services.prisma.applicationChecklist.findFirst({
      where: {
        ...
        checklistId: "TAX_IDENTIFICATION",
      },
      orderBy: { id: 'asc' },
    });
    ...
    if (checklistEntry?.completedAt !== null) {
      const verification = await services.prisma.applicationChecklist.findFirst({
        where: {
          applicationId: applicationId._value,
          checklistId: "TAX_IDENTIFICATION_VERIFICATION",
        },
        orderBy: { id: 'asc' },
      });
    ...
    await services.notificationService.sendToUser(application.getUserId(), {
      ...
      title: new StringValue('Necesitamos que vuelvas a ingresar tu comprobante impositivo'),
      content: new StringValue(`Motivo: ${args.metadata?.message?.toDTO() ?? 'sin especificar'}`),
    });

    await sendFirebaseNotification(application.getUserId()?._value, 'Necesitamos que vuelvas a ingresar tu comprobante impositivo', `Motivo: ${args.metadata?.message?.toDTO() ?? 'sin especificar'}`, services);
