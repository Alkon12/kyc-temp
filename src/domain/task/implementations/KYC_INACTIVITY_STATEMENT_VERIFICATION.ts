...
export const KYC_INACTIVITY_STATEMENT_VERIFICATION: TaskTypeConfig = {
  accept: async (args, services) => {
    ...
    await services.applicationChecklistService.complete({
      ...
      checklistId: ChecklistId.INACTIVITY_STATEMENT_VERIFICATION,
    });
  ...
  decline: async (args, services) => {
    ...
    await services.notificationService.sendToUser(application.getUserId(), {
      ...
      title: new StringValue('Necesitamos que vuelvas a contarnos sobre tu inactividad'),
      content: new StringValue(`Motivo: ${args.metadata?.message?.toDTO() ?? 'sin especificar'}`),
    });

    await sendFirebaseNotification(application.getUserId()?._value, 'Necesitamos que vuelvas a contarnos sobre tu inactividad', `Motivo: ${args.metadata?.message?.toDTO() ?? 'sin especificar'}`, services);
