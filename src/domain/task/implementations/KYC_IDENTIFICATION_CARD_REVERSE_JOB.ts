...
export const KYC_IDENTIFICATION_CARD_REVERSE_JOB: TaskTypeConfig = {
  accept: async (args, services) => {
    ...
    if (contentKey) {
      application.setIdentificationCardReverse(contentKey)
  ...
  decline: async (args, services) => {
    ...
    await services.taskService.create({
      taskTypeId: TaskType.KYC_IDENTIFICATION_CARD_REVERSE_UPLOAD, // TODO: Review reverse doc task: taskTypeId: TaskType.KYC_IDENTIFICATION_CARD_REVERSE_UPLOAD,
      applicationId: application.getId(),
