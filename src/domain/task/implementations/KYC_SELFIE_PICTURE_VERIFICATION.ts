...
export const KYC_SELFIE_PICTURE_VERIFICATION: TaskTypeConfig = {
  accept: async (args, services) => {
    ...
    await services.applicationChecklistService.complete({
      ...
      checklistId: ChecklistId.SELFIE_PICTURE_VERIFICATION,
    });
  ...
  decline: async (args, services) => {
    ...
    if (checklistEntry?.completedAt !== null) {
      const verification = await services.prisma.applicationChecklist.findFirst({
        where: {
          applicationId: applicationId._value,
          checklistId: "SELFIE_PICTURE_VERIFICATION",
        },
        orderBy: { id: 'asc' },
      });
    ...
    await services.notificationService.sendToUser(application.getUserId(), {
      ...
      title: new StringValue('Necesitamos que vuelvas a enviarnos tu foto selfie'),
      content: new StringValue(`Motivo: ${args.metadata?.message?.toDTO() ?? 'sin especificar'}`),
    });

    await sendFirebaseNotification(application.getUserId()?._value, 'Necesitamos que vuelvas a enviarnos tu foto selfie', `Motivo: ${args.metadata?.message?.toDTO() ?? 'sin especificar'}`, services);
