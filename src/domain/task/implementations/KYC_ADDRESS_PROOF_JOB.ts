...
export const KYC_ADDRESS_PROOF_JOB: TaskTypeConfig = {
  accept: async (args, services) => {
    ...
    if (contentKey) {
      application.setAddressProof(contentKey)

      const applicationRepository = container.get<ApplicationRepository>(DI.ApplicationRepository)

      console.log('[KYC_ADDRESS_PROOF_JOB ACCEPT]', application)
    ...
    await services.applicationChecklistService.complete({
      ...
      checklistId: ChecklistId.ADDRESS,
    })
