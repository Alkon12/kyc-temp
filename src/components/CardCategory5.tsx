...
import convertNumbThousand from '@/client/utils/convertNumbThousand'
...
const CardCategory5: FC<CardCategory5Props> = ({ className = '', taxonomy }) => {
  ...
  return (
    <Link href={href} className={`nc-CardCategory5 flex flex-col ${className}`} data-nc-id="CardCategory5">
      ...
      <div className="mt-4 px-3 truncate">
        <h2 className={`text-base sm:text-lg text-neutral-900 dark:text-neutral-100 font-medium truncate`}>{name}</h2>
        <span className={`block mt-2 text-sm text-neutral-6000 dark:text-neutral-400`}>
          {convertNumbThousand(count)} visitas
        </span>
      </div>
    </Link>
  )
