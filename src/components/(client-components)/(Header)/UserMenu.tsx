...
import SwitchDarkMode2 from '@/components/SwitchDarkMode2'
import Link from 'next/link'
import { signOut } from 'next-auth/react'
import useLoginModal from '@/client/hooks/useLoginModal'
import { IUser } from '@/types/IUser'
import imageAnonymous from '@/images/avatars/anonymous.svg'
...
const UserMenu: React.FC<Props> = ({ currentUser, className = '' }) => {
  const loginModal = useLoginModal()
  const backofficeLoginModal = useBackofficeLoginModal()

  const handleBackofficeClick = () => {
    close()
    backofficeLoginModal.onOpen()
  }

  return (
    <>
      <Popover className={`UserMenu relative flex ${className}`}>
        {({ open, close }) => (
          <>
            <Popover.Button
            ...
            >
              <Avatar imgUrl={currentUser ? currentUser.image : imageAnonymous} sizeClass="w-8 h-8 sm:w-9 sm:h-9" />
            </Popover.Button>
            <Transition
              as={Fragment}
                    ...
                    {!currentUser ? (
                      <>
                        ...
                        >
                          ...
                          <div className="ml-4">
                            <p className="text-sm font-medium ">{'Ingresar'}</p>
                          </div>
                        </Link>
                        <Link
                          href={'/#'}
                        ...
                        >
                          ...
                          <div className="ml-4">
                            <p className="text-sm font-medium ">{'Ingresar a Backoffice'}</p>
                          </div>
                        </Link>
                      </>
                    ) : (
                      <>
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="flex-grow">
                            <h4 className="font-semibold">{currentUser.firstName}</h4>
                            <p className="text-xs mt-0.5">{currentUser.email}</p>
                          </div>
                        </div>
