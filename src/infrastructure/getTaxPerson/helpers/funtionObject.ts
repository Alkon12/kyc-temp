...
import { arrayHeaderHard, deleteString } from '../type/getTaxPerson.const'
...
export const myObjectFunction: FunctionMap = {
  ...
  NombredelaLocalidad: (objectValues: DataClientType, value: string[], value2: string) => {
    ...
    if (value.some((item) => item === arrayHeaderHard[3])) {
      ...
      objectValues[value[0]] = ''
    }
    if (value.some((item) => item.includes(deleteString[3]))) objectValues[deleteString[3]] = value[2].trim() ?? ''
